...
# 혹시 prim을 못 찾았을 때만 이 값으로 대체(fallback)한다. >> timeout시 사용
...
class MultiRobotRosBridge(Node):
    ...
    def __init__(self):
        ...
        self.stop_requested = False      # 웹UI "정지" 버튼 수신 플래그
        ...
        # ── 웹UI 제어 토픽 ──────────────────────────────────────────────────
        # 일반 RELIABLE QoS: 버튼 클릭은 단발성 이벤트라 latched 불필요
        ctrl_qos = QoSProfile(
            reliability=ReliabilityPolicy.RELIABLE,
            history=HistoryPolicy.KEEP_LAST,
            depth=1,
        )
        self.start_sub  = self.create_subscription(Bool,    '/start_fueling', self._start_cb,  ctrl_qos)
        self.stop_sub   = self.create_subscription(Bool,    '/stop_fueling',  self._stop_cb,   ctrl_qos)
        self.target_sub = self.create_subscription(Float64, '/fuel_target',   self._target_cb, ctrl_qos)
    ...
    def _start_cb(self, msg: Bool):
        if msg.data:
            ...
            self.get_logger().info("[웹UI] /start_fueling 수신 → Play 트리거 예약")

    def _stop_cb(self, msg: Bool):
        if msg.data:
            self.stop_requested = True
            self.get_logger().info("[웹UI] /stop_fueling 수신 → 주유 즉시 종료 예약")

    def _target_cb(self, msg: Float64):
        self.fuel_target_liters = float(msg.data)
        self.get_logger().info(f"[웹UI] /fuel_target 수신 → 목표 주유량 {self.fuel_target_liters:.2f} L")
...
class RobotARunner:
    ...
    def tick(self, step_count: int):
        ...
        if self.run_state == "WAIT_LOCK_HOLE":
            ...
            if mean is not None:
                # 주유구 위치가 확정된 순간 시퀀스를 만들고 바로 실행 상태로 전환
            ...
            if self.wait_steps >= WAIT_LOCK_TIMEOUT_STEPS:
                ...
                return
            return
        # 디버깅 마거를 사용중이라면 마커중 주유구 마커위치를 잦아서 이동한다.
...
def main():
    ...
    def _force_exit(*_):
        if rclpy.ok():
            try:
                ...
                pass
        _os._exit(0)
    # Ctrl+C(SIGINT) 또는 종료 신호(SIGTERM)를 받았을 때 _force_exit를 실행하도록 등록하는 코드
    # saac Sim은 종료 시 내부적으로 SIGSEGV(세그폴트)가 발생하는 버그가 있어서, 정상적인 Python 종료 절차를 거치면 오류 메시지가 쏟아집니다.
    #  _force_exit는 그냥 os._exit(0)으로 프로세스를 강제 즉시 종료해서 그 버그를 우회
    ...
    try:
        ...
        # Python 예외 계층의 최상위 클래스
        import traceback as _tb
        # traceback으로 전체 스택을 출력해 디버그 정보를 남깁니다.
        print(f"\n[LOOP ERROR] {type(_exc).__name__}: {_exc}", flush=True)
        _tb.print_exc()
    finally:
        print(f"\n[LOOP] 루프 종료 → _force_exit 호출\n", flush=True)
        _force_exit()
        # 정상 종료든 에러 종료든 항상 깔끔하게 프로세스를 끝내기 위한 안전망입니다.
