...
from std_msgs.msg import Bool, String, Float64  # 참/거짓, 문자열, 실수 메시지 타입
...
ALLEGRO_POSES = {
    ...
    "press": {
        "index":  [0.0300, 0.2700, 0.3000, 0.5000],
        "middle":  [0.0000, 0.2700, 0.3000, 0.5000],
        "ring":  [-0.0300, 0.2700, 0.3000, 0.5000],
...
# 처음에 빠르게 가까워질수록 자동으로 느려지게 하여 제어하기 위해 
...
def build_close_cover_sequence(
    ...
    stages += [
        ...
        FuelStage("B10_02_forward_entry", forward_entry, hold_steps=8, tolerance=0.08, speed=NEAR_TARGET_SPEED,
    ...
    if center_reference_point is not None:
        ...
        stages += [
            FuelStage("B10_05_retreat_for_press", retreat_point, tolerance=0.12, speed=RETREAT_TARGET_SPEED),
            FuelStage("B10_06_center_align_press", center_align_point, hold_steps=10, tolerance=0.12, speed=NEAR_TARGET_SPEED),
...
class RobotARunner:
    ...
    def tick(self, step_count: int):
        ...
        if self.run_state == "RUN_SEQUENCE":
            ...
            # 웹UI "정지" 버튼: 삽입 단계에서 받으면 hold_count를 강제로 채워
            # WaypointSequence가 다음 스텝에서 자연스럽게 다음 단계(후퇴)로 넘어가게 한다.
            if stage.name == "04_insert_into_cylinder" and self.ros_bridge.stop_requested:
                self.ros_bridge.stop_requested = False
                self.sequence.hold_count = stage.hold_steps
                print("\n[A] 웹UI 정지 신호 → 주유 즉시 종료, 노즐 회수 시작\n", flush=True)
    ...
    def tick(self, step_count: int):
        ...
        if self.run_state == "GRIP_UNSCREW":
            if self.sub_phase == "close_grip":
                ...
                # return
                if self.gripper_hold_count >= GRIPPER_ACTION_HOLD_STEPS:
                    # [TIMEOUT_FIX] align_grasp_center/cap_center_roll 실험 경로 비활성화.
                    # 이유:
                    # - cap을 잡은 뒤 RMPFlow로 중지-엄지 중심을 다시 맞추는 추가 이동이 들어가면
                    #   원래 cap 분리 시퀀스보다 자세가 크게 틀어질 수 있다.
                    # - 그 상태가 이후 RESTORE_CAP/B9_extract_retreat timeout으로 이어질 수 있어,
                    #   Allegro hand로 cap을 잡는 것까지만 유지하고 기존 joint_6 회전 흐름으로 복구한다.
                    #
                    # 기존 실험 경로 보존:
                    # self.cap_grasp_align_steps = 0
                    ...
                    self.sub_phase = "rotate"
                    print(
                        f"\n[B] Allegro hand로 마개 고정 완료 -> 기존 joint_6 방식으로 마개 풀기 시작 "
                        f"(목표 {abs(np.degrees(UNSCREW_TOTAL_ANGLE_RAD)):.0f}도)\n",
                        flush=True,
                    )
                    return
        ...
        if self.run_state == "RESTORE_CAP":
            if self.sub_phase == "insert":
                ...
                if done:
                    ...
                    # [TIMEOUT_FIX] 조이기는 기존 joint_6 단독 회전으로 수행한다.
                    # 이유:
                    # - 현재 screw sub_phase는 아래에서 self._rotate_robot_joint6_only()만 사용한다.
                    # - 따라서 _begin_cap_center_roll() 기준 캡처는 실제 screw 제어에 쓰이지 않는
                    #   잔여 실험 코드이며, 로그와 원인 추적을 헷갈리게 만든다.
                    #
                    # 기존 실험 코드 보존:
                    ...
                    self.sub_phase = "screw"
                    print(
                        f"\n[B] 마개 삽입 위치 도착 -> 기존 joint_6 방식으로 마개 조이기 시작 "
                        f"(목표 {abs(np.degrees(UNSCREW_TOTAL_ANGLE_RAD)):.0f}도)\n",
                        flush=True,
                    )
                return
