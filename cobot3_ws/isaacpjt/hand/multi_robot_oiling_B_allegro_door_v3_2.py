...
ALLEGRO_POSES = {
    ...
    "press": {
        "index":  [0.0300, 0.4500, 0.4000, 0.5000],
        "middle":  [0.0000, 0.4500, 0.4000, 0.5000],
        "ring":  [-0.0300, 0.4500, 0.4000, 0.5000],
...
# B9 마개 복원/후퇴 이후, B10 덮개 닫기 진입 직전에 팔끝 꼬임을 풀기 위한 짧은 관절 정렬.
# 전체 home 복귀가 아니라 joint_4~6만 초기 각도 쪽으로 천천히 정렬해서 link_5/link_6 뒤틀림만 완화한다.
PRE_CLOSE_JOINT_ALIGN_STEPS = 80
PRE_CLOSE_JOINT_ALIGN_ALPHA = 0.06
PRE_CLOSE_JOINT_ALIGN_TOLERANCE = 0.04
PRE_CLOSE_ALIGN_JOINT_NAMES = ["joint_4", "joint_5", "joint_6"]
...
DOOR_FULL_CLOSE_RETREAT_DISTANCE = 0.16
...
DOOR_CLOSE_OUTWARD_CLEARANCE = 0.18
...
def build_close_cover_sequence(
    ...
    start_push = p(COVER_START_DEG) + right_vec * 00.15

    stages += [
        FuelStage("B10_01_move_right", right_ready, tolerance=0.08, speed=DEFAULT_TARGET_SPEED, use_orientation=False),
        FuelStage("B10_02_forward_entry", forward_entry, hold_steps=8, tolerance=0.08, speed=NEAR_TARGET_SPEED,use_orientation=False,
                  target_door_angle=COVER_OPEN_DEG),
        FuelStage("B10_03_push_left_mid", mid_push, hold_steps=20, tolerance=0.07, speed=NEAR_TARGET_SPEED,use_orientation=False,
                  target_door_angle=mid_deg),
        FuelStage("B10_04_push_left_start", start_push, hold_steps=30, tolerance=0.11, speed=NEAR_TARGET_SPEED,
    ...
    if center_reference_point is not None:
        ...
        stages += [
            ...
            FuelStage("B10_07_final_press", full_close_point, hold_steps=30, tolerance=0.05,
...
class RobotBRunner:
    ...
    def __init__(self, robot, controller, ros_bridge: MultiRobotRosBridge, camera_prim_path, task: MultiRobotOilingTask):
        ...
        # B9 후퇴 완료 후 B10 문 닫기 전에 joint_4~6만 짧게 정렬하기 위한 상태값.
        self.pre_close_align_indices = None
        self.pre_close_align_target = None
        self.pre_close_align_count = 0
    ...
    def _prepare_pre_close_joint_alignment(self):
        """B10 덮개 닫기 직전, 전체 home 복귀 대신 손목부 joint만 짧게 정렬한다."""
        current = self.robot.get_joint_positions()
        home_like = build_initial_joint_positions(
            self.robot,
            current,
            INITIAL_ARM_JOINT_DEG_B,
        )
        ...
        indices = []
        targets = []
        for joint_name in PRE_CLOSE_ALIGN_JOINT_NAMES:
            idx = find_dof_index(self.robot, joint_name)
            if idx is None:
                print(f"[B][PRE_CLOSE_ALIGN][WARN] {joint_name} index를 찾지 못함", flush=True)
                continue
            indices.append(idx)
            targets.append(home_like[idx])

        self.pre_close_align_indices = np.array(indices, dtype=np.int32)
        self.pre_close_align_target = np.array(targets, dtype=float)
        self.pre_close_align_count = 0
        ...
        print(
            f"\n[B] 덮개 닫기 전 joint 정렬 시작: joints={PRE_CLOSE_ALIGN_JOINT_NAMES}, "
            f"steps={PRE_CLOSE_JOINT_ALIGN_STEPS}\n",
            flush=True,
        )

    def _step_pre_close_joint_alignment(self) -> bool:
        """선택된 joint_4~6만 천천히 초기 각도 쪽으로 보낸다."""
        if self.pre_close_align_indices is None or len(self.pre_close_align_indices) == 0:
            return True

        current = self.robot.get_joint_positions()
        cur_sel = current[self.pre_close_align_indices]
        target_sel = self.pre_close_align_target
        next_sel = cur_sel + PRE_CLOSE_JOINT_ALIGN_ALPHA * (target_sel - cur_sel)

        self.robot.apply_action(
            ArticulationAction(
                joint_positions=next_sel,
                joint_indices=self.pre_close_align_indices,
            )
        )

        if USE_B_ALLEGRO_WRIST:
            self._set_wrist_pitch(WRIST_PITCH_NEUTRAL_DEG)
            self._set_hand_pose(HAND_POSE_OPEN)

        self.pre_close_align_count += 1
        err = float(np.linalg.norm(target_sel - next_sel))

        if err < PRE_CLOSE_JOINT_ALIGN_TOLERANCE:
            print(f"\n[B] 덮개 닫기 전 joint 정렬 완료: err={err:.4f}\n", flush=True)
            return True

        if self.pre_close_align_count >= PRE_CLOSE_JOINT_ALIGN_STEPS:
            print(f"\n[B][PRE_CLOSE_ALIGN] 정렬 시간 종료: err={err:.4f} -> B10 진행\n", flush=True)
            return True
        ...
        if self.run_state == "RESTORE_CAP":
            ...
            if self.sub_phase == "open_grip":
                ...
                if self.gripper_hold_count >= GRIPPER_ACTION_HOLD_STEPS:
                    ...
                    retreat_target = ee_pos + PORT_OUTWARD_NORMAL_UNIT * 0.10
                    self.sequence = single_stage_sequence(
                        "B9_extract_retreat", retreat_target,
                        hold_steps=15, tolerance=0.08, speed=RETREAT_TARGET_SPEED,
            ...
            if self.sub_phase == "retreat":
                ...
                if done:
                    # B10으로 바로 넘어가지 않고, 기존 잠깐 멈추는 구간을 joint_4~6 정렬 구간으로 사용한다.
                    self._prepare_pre_close_joint_alignment()
                    self.sub_phase = "pre_close_joint_align"
                return

            if self.sub_phase == "pre_close_joint_align":
                self._hold_gripper(closed=False)

                aligned = self._step_pre_close_joint_alignment()
                if not aligned:
                    return

                # 정렬이 끝난 뒤의 EE orientation을 B10 기준 orientation으로 다시 잡는다.
                ee_pos_now, ee_ori_now = self.robot.end_effector.get_world_pose()
                self.locked_target_orientation = ee_ori_now.copy()
                ...
                # 여기서 바로 door bent pose를 적용하면 방금 한 정렬을 다시 망가뜨릴 수 있다.
                # B10 stage별 wrist/hand pose는 _drive_close_cover_sequence() 안에서 적용한다.
                if USE_B_ALLEGRO_WRIST:
                    self._set_wrist_pitch(WRIST_PITCH_NEUTRAL_DEG)
                    self._set_hand_pose(HAND_POSE_OPEN)
