...
CAR_ARRIVAL_START_OFFSET = np.array([10.0, 0.0, 0.0], dtype=float)  # 출발 지점 = 도착 지점 + 이 오프셋(m)
...
def build_fuel_port_sequence(fuel_port_center: np.ndarray) -> WaypointSequence:
    ...
    stage_log = {
        ...
        "05_retreat_near": "A 주유 완료, 노즐 후퇴 시작",
        "06_retreat_mid": "A 노즐 후퇴 중(중간 지점)",
        "07_retreat_far": "A 노즐 후퇴 완료",
    }
...
def build_close_cover_sequence(
    ...
    stage_log.update({
        "B10_01_reengage_open": f"B 덮개 재접촉({COVER_OPEN_DEG:.0f}도), 닫기 시작",
        "B10_02_push_mid": f"B 덮개 밀기 중({mid_deg:.0f}도)",
        "B10_03_push_start": f"B 덮개 {COVER_START_DEG:.0f}도까지 닫음",
    })
    ...
    if center_reference_point is not None:
        ...
        stage_log.update({
            "B10_04_retreat": f"B 덮개 {COVER_START_DEG:.0f}도까지 닫고 후퇴",
            "B10_05_center_align": "B 덮개 중앙 정렬",
            "B10_06_full_close_push": f"B 덮개 완전 닫힘({COVER_FULL_CLOSE_DEG:.0f}도)",
        })
    return WaypointSequence(stages, stage_log)
...
class CarArrivalController:
    ...
    def __init__(self, car_prim_path: "str | None"):
        ...
        if self.car_prim_path is None:
            print(f"  [CAR][WARN] '{CAR_ROOT_PRIM_NAME}' prim을 못 찾아서 슬라이드-인 시뮬레이션을 건너뜀")
...
class MultiRobotOilingTask(BaseTask):
    ...
    def hide_debug_markers(self):
        """fuel_marker_door/cap/hole 디버그 박스를 숨긴다. Play를 누르면(실제 시뮬레이션 시작)
        더 이상 위치 확인용 박스가 필요 없으므로 시야를 가리지 않게 꺼둔다."""
...
class RobotARunner:
    ...
    def on_play_reset(self):
        ...
        print("[A] 대기 중: B가 덮개를 열고 마개를 분리할 때까지 기다립니다")
...
class RobotBRunner:
    ...
    def on_play_reset(self):
        ...
        print("[B] 덮개 자동 열림 시작, 문이 열리는 동안 대기합니다 (mode=door)")
...
def main():
    ...
    print(f"  [CAR] car_prim_path = {car_prim_path}")
    ...
    while simulation_app.is_running():
        ...
        if is_playing:
            ...
            if not car_controller.is_done():
                ...
                # (기존 RobotARunner/RobotBRunner 내부 상태머신은 전혀 건드리지 않고, 그냥
                # tick 호출 자체를 미뤄서 시작 시점만 뒤로 늦춘다).
