...
DEFAULT_TARGET_SPEED = 0.180   # 평범하게 이동할 때
NEAR_TARGET_SPEED    = 0.120   # 목표에 가까워졌을 때 (더 정밀하게, 천천히)
INSERT_TARGET_SPEED  = 0.060   # 구멍에 삽입할 때 (충돌 위험 있어 가장 느리게)
RETREAT_TARGET_SPEED = 0.150   # 빼거나 후퇴할 때
...
GRIPPER_LENGTH_B = 0.32  # B 로봇 그리퍼 길이(m) - 손목(link_6)에서 손가락 끝까지 거리.
                         # 그리퍼 USD scale을 (1,1,1)->(2,2,2)로 키워서 실제 길이도 2배(0.16->0.32)가 됐다.
...
DOOR_CLOSE_OUTWARD_CLEARANCE = 0.3
...
def build_close_cover_sequence(
    ...
    """B 로봇이 커버를 120도(완전열림)에서 30도(닫힘 기준)까지 밀어서 닫는 시퀀스.
    door_reference_point: COVER_START_DEG(30도) 기준의 커버 표면 위 한 점.
    pivot/axis: 힌지(RevoluteJoint)의 회전 중심점과 회전축.
    avoid_point: 마개를 복원한 자리에서 곧장 열린 덮개로 다가가면 손이 덮개를 뚫고 지나가므로,
    build_cap_approach_sequence의 B5_00_avoid_door와 같은 경유점을 먼저 거쳐서 돌아가게 한다.
    center_reference_point: 주어지면 30도까지 닫은 뒤 한 번 빠져나와(retreat) 도어 중앙으로
    정렬(center_align)한 다음, 그 중앙 지점을 0도(완전 닫힘)까지 직선으로 밀어주는 마무리
    3단계(B10_04~06)를 추가한다. None이면 기존과 동일하게 30도에서 끝난다."""
    def p(angle_deg):
    ...
    mid_deg = (COVER_OPEN_DEG + COVER_START_DEG) / 2.0  # 120, 30 -> 75
    stages = []
    stage_log = {}
    if avoid_point is not None:
        stages.append(FuelStage("B10_00_avoid_door", avoid_point, tolerance=0.08, speed=DEFAULT_TARGET_SPEED))
        stage_log["B10_00_avoid_door"] = "B 덮개 회피(복귀)"
    stages += [
        ...
        FuelStage("B10_02_push_mid", p(mid_deg), hold_steps=20, tolerance=0.07, speed=NEAR_TARGET_SPEED,
                  target_door_angle=mid_deg),
        FuelStage("B10_03_push_start", p(COVER_START_DEG), hold_steps=30, tolerance=0.07, speed=NEAR_TARGET_SPEED,
                  target_door_angle=COVER_START_DEG),
    ]
    stage_log.update({
        "B10_01_reengage_open": f"B 커버 재접촉({COVER_OPEN_DEG:.0f}도)",
        "B10_02_push_mid": f"B 커버 {mid_deg:.0f}도(닫는중)",
        "B10_03_push_start": "B 커버 30도 닫힘완료",
    })

    if center_reference_point is not None:
        retreat_point = p(COVER_START_DEG) + PORT_OUTWARD_NORMAL_UNIT * DOOR_FULL_CLOSE_RETREAT_DISTANCE
        ...
        stages += [
            FuelStage("B10_04_retreat", retreat_point, tolerance=0.06, speed=RETREAT_TARGET_SPEED),
            FuelStage("B10_05_center_align", center_align_point, hold_steps=10, tolerance=0.06, speed=NEAR_TARGET_SPEED),
            FuelStage("B10_06_full_close_push", full_close_point, hold_steps=30, tolerance=0.05,
                      speed=NEAR_TARGET_SPEED, target_door_angle=COVER_FULL_CLOSE_DEG),
        ]
        stage_log.update({
            "B10_04_retreat": "B 덮개 30도 닫고 후퇴",
            "B10_05_center_align": "B 덮개 중앙 정렬",
            "B10_06_full_close_push": "B 덮개 0도(완전닫힘) 밀기",
        })
    return WaypointSequence(stages, stage_log)
...
def build_cap_depth_stages(center: np.ndarray, far_name: str, near_name: str, insert_name: str) -> list:
    ...
    return [
        ...
        FuelStage(insert_name, center + PORT_OUTWARD_NORMAL_UNIT * (GRIPPER_LENGTH_B + 0.15),
...
class MultiRobotOilingTask(BaseTask):
    ...
    def _setup_door_drive(self):
        ...
        if self.door_joint_prim is None:
            print("  [COVER][WARN] door_joint_prim이 없어 덮개 드라이브 설정을 건너뜀")
            return
...
# (덮개 열기는 코드가 구동하지 않고 USD 씬 설정으로 이미 열린 상태에서 시작한다
#  - WAIT_YELLOW/OPEN_COVER/RETURN_MID 제거됨)
# ============================================================
class RobotBRunner:
    ...
    def __init__(self, robot, controller, ros_bridge: MultiRobotRosBridge, camera_prim_path, task: MultiRobotOilingTask):
        ...
        self.camera_prim_path = camera_prim_path
        self.task = task
        self.run_state = "WAIT_BLUE"  # 덮개는 이미 USD 씬 설정으로 열려있어 마개(blue) lock부터 시작
        self.sub_phase = None
        self.lock_acquirer = None
        self.sequence = None
    ...
    def on_play_reset(self):
        ...
        곧바로 카메라에 "파란색(마개)부터 찾아"라고 명령(mode_switch=blue)을 보낸다
        (덮개는 이미 USD 씬 설정으로 120도까지 열려있다).
        이전 판 CLOSE_COVER에서 0으로 꺼뒀을 수 있는 덮개 자동 열기 velocity 드라이브를
        다시 DOOR_AUTO_OPEN_DRIVE_VELOCITY로 복원해, 이번 판에도 자동 열기가 동작하게 한다."""
        self.task.set_door_auto_open_drive_velocity(DOOR_AUTO_OPEN_DRIVE_VELOCITY)
        _, ee_ori = self.robot.end_effector.get_world_pose()
        self.locked_target_orientation = ee_ori.copy()
        ...
        print("[B] RESET -> WAIT_BLUE (mode_switch=blue 발행)")
    ...
    def tick(self, step_count: int):
        ...
        # ---------------- B-4: WAIT_BLUE ----------------
        # 덮개는 이미 USD 씬 설정으로 120도까지 열려있으므로, 바로 파란색(마개) lock부터 시작한다.
        if self.run_state == "WAIT_BLUE":
            ...
            if self.wait_steps >= WAIT_LOCK_TIMEOUT_STEPS:
                ...
                return
            if step_count % PRINT_EVERY_N_STEPS == 0:
                # 덮개가 USD 씬 설정대로 120도 쪽에 열려있는지 이 구간에서 같이 찍어서,
                # 안 맞으면 바로 알아챌 수 있게 한다.
                print(f"[B][WAIT_BLUE] door_angle={self.task.current_door_angle_deg():.1f} "
                      f"locked={self.ros_bridge.target_locked} "
                      f"pose_count={self.ros_bridge.pose_count} lock_count={self.ros_bridge.lock_count} "
                      f"samples={len(self.lock_acquirer.samples)}/{CONTROLLER_REQUIRED_LOCK_SAMPLES}")
            return
        ...
        if self.run_state == "RESTORE_CAP":
            ...
            if self.sub_phase == "retreat":
                ...
                if done:
                    ...
                    door_close_push_point = self.task.door_world_position.copy()
                    ...
                    self.sequence = build_close_cover_sequence(
                        ...
                        center_reference_point=self.task.door_world_position.copy(),
                    )
                    self.sub_phase = None
                    ...
                    print(f"\n[B-9] 마개 복원 완료 -> CLOSE_COVER "
                          f"(덮개 각도는 로봇 모션에 맞춰 {COVER_OPEN_DEG:.0f}->{COVER_START_DEG:.0f}->"
                          f"{COVER_FULL_CLOSE_DEG:.0f}도로 동기화)\n")
            return
